...
gis_write_error("trx.complete", $cfg);
    ...
    if (isset($data["currency"]) && $data["currency"]) {
        if (isset($data["trx_id"]) && $data["trx_id"]) {
            if (isset($data["amount"]) && is_numeric($data["amount"])) {
                if (gis_check_sign($data, "trx.complete", $cfg)) {
                    $response = complete_trx(
...
if (is_debug() && rand(0, 99) < 10) {
    if (rand(0, 9) < 5) {
        gis_write_error("trx.complete - random 500 err", $cfg);
        http_response_code(500);
    } else {
        gis_write_error("trx.complete - random sleep err", $cfg);
        sleep(3);
    }
}
...
if (is_array($response)) {
    ...
    echo '{"method":"trx.complete","status":200,"response":' . json_encode($response) . '}';
} else {
    // ошибка отмены транзакции
    gis_write_error("trx.complete - " . clean_string($response), $cfg);
    echo '{"method":"trx.complete","status":500,"message":"' . clean_string($response) . '"}';
}